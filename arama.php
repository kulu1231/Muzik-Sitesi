...
<div class="icerikkap sarkilar"><br /><br />
<?php
error_reporting(0);
$ara = $_POST["ara"];
$sarki="select sanatci_adi,sarki_adi,sarkilink from sarki where sarki_adi like '%$ara%' or sanatci_adi like '%$ara%'";
$statement = $baglanti->prepare($sarki);
  $statement->execute();
   $statement->bind_result($sanatci,$sarki,$link);
...
while($statement->fetch())
{
	...
	echo '<div style="float:left;margin-left:33px;width:175px;margin-bottom:50px;border:1px solid #CCC">';
	 ...
	 echo '<a>'.$a." - ".$b."</a>";
	  ...
	  echo '<center>'.'<a style="float:left" class="a" href="arama.php?s='.$link.'">Dinle</a>'.'&nbsp;'.'<a class="b" style="float:right" target="_blank" href="'.$link.'">İndir</a>'.'</center>';
	  echo "</div>";
	  echo '</div>';
